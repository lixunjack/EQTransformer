...
def trainer(input_hdf5=None,
    ...
    def train(args):
        ...
        Below is Jack's own implementation of the train function.
        TL_model=False, 
...
# The above code is compiling a model in Python. It is specifying the loss function to be used, the
# loss weights, the optimizer (Adam with a learning rate of 0.001), and the metrics to be evaluated
# (f1 score).
            model.compile(loss = args['loss_types'],
                  loss_weights =  args['loss_weights'],           
                  optimizer = Adam(lr = 0.001),
                  metrics = [f1])
            print('*** Loading pre-trained is complete!', flush=True)  
        else:
